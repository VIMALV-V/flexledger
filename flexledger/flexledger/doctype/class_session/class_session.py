...
class ClassSession(Document):
    ...
    def validate(self):
        ...
        for row in self.attendees:
            ...
            if package_member != row.member:
                frappe.throw(
                    f"Package Purchase {row.package_purchase} does not "
                    f"belong to member {row.member}."
                )
            ...
            if package.status != "Active":
                frappe.throw(
                    f"Package {row.package_purchase} for member "
                    f"{row.member} is not Active."
                )

            if (
                package.expiry_date
                and self.session_date
                and package.expiry_date < self.session_date
            ):
                frappe.throw(
                    f"Package {row.package_purchase} for member "
                    f"{row.member} has expired."
                )
            ...
            if package.credits_remaining < row.credits_charged:
                frappe.throw(
                    f"Member {row.member} has only "
                    f"{package.credits_remaining} credits remaining, "
                    f"but {row.credits_charged} credits are required."
                )
    ...
    def on_update(self):
        ...
        # Calculations should be performed in validate(),
        # while controlled database updates can be used when needed.
        pass
